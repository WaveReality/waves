...
func schrodOmega(ss *Sim) float32 {
	return 2 * math32.Pi / SchrodingerPeriod
}

// SchrodingerPeriod is the harmonic oscillator period in leapfrog steps.
const SchrodingerPeriod float32 = 320
...
func HydrogenGround(ss *Sim) {
	...
	ss.CopyCurToPrev() // InvR writes cur only, and a potential that is there on
	// one step and gone the next pumps the wave straight to infinity
...
func HydrogenP(ss *Sim) {
	p := ss.Params
	a := HydrogenRadius / 2 // n = 2 decays over 2a, so halve a to keep the size
	k := p.Hbar * p.Hbar / (p.Mass * a)
	ctr := math32.Vec3(-1, -1, -1)
	ss.InvR(CabV, ctr, -k)
	ss.CopyCurToPrev()
...
var SchrodingerShouldDisplay = []string{"Edges", "Hbar", "Mass", "Wavelength", "PacketWidth", "Amplitude"}
...
func (ss *Sim) SchrodingerStats() {
	...
	ss.AddStat(ss.StatGroupVel(math32.X, CabMag))
}
