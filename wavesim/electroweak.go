...
func ElectroweakKernel(i uint32) { //gosl:kernel
	...
	w1xv := State.Value(int(z), int(y), int(x), int(EWW1Xv), int(prv))
	w1yv := State.Value(int(z), int(y), int(x), int(EWW1Yv), int(prv))
	w1zv := State.Value(int(z), int(y), int(x), int(EWW1Zv), int(prv))
	w2xv := State.Value(int(z), int(y), int(x), int(EWW2Xv), int(prv))
	w2yv := State.Value(int(z), int(y), int(x), int(EWW2Yv), int(prv))
	w2zv := State.Value(int(z), int(y), int(x), int(EWW2Zv), int(prv))
	w3xv := State.Value(int(z), int(y), int(x), int(EWW3Xv), int(prv))
	w3yv := State.Value(int(z), int(y), int(x), int(EWW3Yv), int(prv))
	w3zv := State.Value(int(z), int(y), int(x), int(EWW3Zv), int(prv))
...
func EWWStep(x, y, z, cur, prv, nu int32, csq float32, om, src math32.Vector3, boris, noWave bool) {
	vs := int32(EWW10s) + nu
	vv := int32(EWW10v) + nu
	p1 := State.Value(int(z), int(y), int(x), int(vs), int(prv))
	p2 := State.Value(int(z), int(y), int(x), int(vs+8), int(prv))
	p3 := State.Value(int(z), int(y), int(x), int(vs+16), int(prv))
	...
	q1 := State.Value(int(z), int(y), int(x), int(vv), int(prv))
	q2 := State.Value(int(z), int(y), int(x), int(vv+8), int(prv))
	q3 := State.Value(int(z), int(y), int(x), int(vv+16), int(prv))
	...
	State.Set(vn.X, int(z), int(y), int(x), int(vv), int(cur))
	State.Set(vn.Y, int(z), int(y), int(x), int(vv+8), int(cur))
	State.Set(vn.Z, int(z), int(y), int(x), int(vv+16), int(cur))
	State.Set(p1+vn.X, int(z), int(y), int(x), int(vs), int(cur))
	State.Set(p2+vn.Y, int(z), int(y), int(x), int(vs+8), int(cur))
	State.Set(p3+vn.Z, int(z), int(y), int(x), int(vs+16), int(cur))
}
...
// ewNoise seeds a state variable with smooth random fluctuation: a sum of a few
// long-wavelength modes rather than per-site white noise.
//
// White noise would be the obvious choice and is the wrong one. It carries equal
// power at every wavenumber including the lattice cutoff, which is both
// unphysical -- nothing in the continuum theory excites the grid scale -- and
// the fastest way to destabilise a nonlinear solver. Using whole numbers of
// wavelengths per box also keeps the seed periodic, which matters with wrapped
// edges.
func ewNoise(ss *Sim, vr EWStates, amp float32, nmodes int, rnd *rand.Rand) {
	vri := int(vr.Int64())
	ctx := GetCtx(0)
	cur := ctx.CurState
	prv := ctx.PrevState()
	...
	for range nmodes {
		nx := float32(rnd.IntN(3) + 1)
		ny := float32(rnd.IntN(3) + 1)
		nz := float32(rnd.IntN(3) + 1)
		ph := TwoPi * rnd.Float32()
...
func HiggsSymmetric(ss *Sim) {
	...
	ewNoise(ss, EWHsCa, amp, 6, rnd)
	ewNoise(ss, EWHsCb, amp, 6, rnd)
	ewNoise(ss, EWHs0a, amp, 6, rnd)
	ewNoise(ss, EWHs0b, amp, 6, rnd)
}
...
func ZPulse(ss *Sim) {
	...
	HiggsBroken(ss)
	p := ss.Params
	wl := float32(16) // same carrier as PhotonPulse, so the two are comparable
...
// peaking once.
//
// The amplitude is deliberately large, a tenth of the vacuum value, because the
// effect is quadratic in W and vanishes for small packets.
func WCollision(ss *Sim) {
	...
	ss.Config.Wavelength = wl
	ss.Config.PacketWidth = 1.2 * wl
	ss.SlabPacketConfig(EWW1Xs, EWW1Xv, zl*0.3, amp, 1, om)
	ss.SlabPacketConfig(EWW2Xs, EWW2Xv, zl*0.7, amp, -1, om)
}
