...
var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.EMKernel", Doc: "EMKernel is the kernel for computing the EM equations,\non EM state values (EMStates).", Directives: []types.Directive{{Tool: "gosl", Directive: "kernel"}}, Args: []string{"i"}})
...
var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.RunEMKernel", Doc: "RunEMKernel runs the EMKernel kernel with given number of elements,\non either the CPU or GPU depending on the UseGPU variable.\nCan call multiple Run* kernels in a row, which are then all launched\nin the same command submission on the GPU, which is by far the most efficient.\nMUST call RunDone (with optional vars to sync) after all Run calls.\nAlternatively, a single-shot RunOneEMKernel call does Run and Done for a\nsingle run-and-sync case.", Args: []string{"n"}})

var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.RunEMKernelGPU", Doc: "RunEMKernelGPU runs the EMKernel kernel on the GPU. See [RunEMKernel] for more info.", Args: []string{"n"}})

var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.RunEMKernelCPU", Doc: "RunEMKernelCPU runs the EMKernel kernel on the CPU.", Args: []string{"n"}})

var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.RunOneEMKernel", Doc: "RunOneEMKernel runs the EMKernel kernel with given number of elements,\non either the CPU or GPU depending on the UseGPU variable.\nThis version then calls RunDone with the given variables to sync\nafter the Run, for a single-shot Run-and-Done call. If multiple kernels\ncan be run in sequence, it is much more efficient to do multiple Run*\ncalls followed by a RunDone call.", Args: []string{"n", "syncVars"}})
