...
var _ = types.AddType(&types.Type{Name: "github.com/WaveReality/waves/wavesim.HiggsStates", IDName: "higgs-states", Doc: "HiggsStates are the state variables for the Higgs Model."})
...
var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.RunHiggsKernel", Doc: "RunHiggsKernel runs the HiggsKernel kernel with given number of elements,\non either the CPU or GPU depending on the UseGPU variable.\nCan call multiple Run* kernels in a row, which are then all launched\nin the same command submission on the GPU, which is by far the most efficient.\nMUST call RunDone (with optional vars to sync) after all Run calls.\nAlternatively, a single-shot RunOneHiggsKernel call does Run and Done for a\nsingle run-and-sync case.", Args: []string{"n"}})

var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.RunHiggsKernelGPU", Doc: "RunHiggsKernelGPU runs the HiggsKernel kernel on the GPU. See [RunHiggsKernel] for more info.", Args: []string{"n"}})

var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.RunHiggsKernelCPU", Doc: "RunHiggsKernelCPU runs the HiggsKernel kernel on the CPU.", Args: []string{"n"}})

var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.RunOneHiggsKernel", Doc: "RunOneHiggsKernel runs the HiggsKernel kernel with given number of elements,\non either the CPU or GPU depending on the UseGPU variable.\nThis version then calls RunDone with the given variables to sync\nafter the Run, for a single-shot Run-and-Done call. If multiple kernels\ncan be run in sequence, it is much more efficient to do multiple Run*\ncalls followed by a RunDone call.", Args: []string{"n", "syncVars"}})
...
var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.HiggsKernel", Doc: "HiggsKernel is the kernel for computing Higgs model.\nOnly supports 3D.", Directives: []types.Directive{{Tool: "gosl", Directive: "kernel"}}, Args: []string{"i"}})

var _ = types.AddFunc(&types.Func{Name: "github.com/WaveReality/waves/wavesim.HiggsViewAll", Doc: "HiggsViewAll configures the View to display Higgs values", Args: []string{"view"}})
