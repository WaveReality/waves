...
//////// import: "wavecdir.go"
fn WaveCDirKernel(i: u32) { //gosl:kernel
	let ctx = Ctx[0];
	var x: i32;
	var y: i32;
	var z: i32;
	...
	var oa = StateGet(Index5D(TensorStrides[40], TensorStrides[41], TensorStrides[42], // psi(t-1), not yet overwritten
	TensorStrides[43], TensorStrides[44], u32(z), u32(y), u32(x), u32(WaveCa), u32(cur)));
	var ob = StateGet(Index5D(TensorStrides[40], TensorStrides[41], TensorStrides[42], TensorStrides[43], TensorStrides[44], u32(z), u32(y), u32(x), u32(WaveCb), u32(cur)));
	var na = StateGet(Index5D(TensorStrides[40], TensorStrides[41], TensorStrides[42], // psi(t)
	TensorStrides[43], TensorStrides[44], u32(z), u32(y), u32(x), u32(WaveCa), u32(prv)));
	var nb = StateGet(Index5D(TensorStrides[40], TensorStrides[41], TensorStrides[42], TensorStrides[43], TensorStrides[44], u32(z), u32(y), u32(x), u32(WaveCb), u32(prv)));
	var ga: vec3<f32>;
	var gb: vec3<f32>;
	if (Params[0].ThreeD == 1) {
		ga = Gradient10(x, y, z, i32(WaveCa), prv);
		gb = Gradient10(x, y, z, i32(WaveCb), prv);
	} else {
		ga = Gradient1D(x, y, z, i32(WaveCa), prv);
		gb = Gradient1D(x, y, z, i32(WaveCb), prv);
	}
	...
	StateSet(na*pa + nb*pb, Index5D(TensorStrides[40], TensorStrides[41], TensorStrides[42],
	TensorStrides[43], TensorStrides[44], u32(z), u32(y), u32(x), u32(WaveCMag), u32(cur)));
}
