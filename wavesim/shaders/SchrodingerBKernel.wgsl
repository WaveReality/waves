...
fn SchrodingerBKernel(i: u32) { //gosl:kernel
	let ctx = Ctx[0];
	var x: i32;
	var y: i32;
	var z: i32;
	...
	var posA = StateGet(Index5D(TensorStrides[40], TensorStrides[41], TensorStrides[42], // the new a: this is the whole point
	TensorStrides[43], TensorStrides[44], u32(z), u32(y), u32(x), u32(CabAs), u32(cur)));
	var hbar = Params[0].Hbar;
	var hm = Params[0].HSqOver2M / hbar;
	...
	if (Params[0].ThreeD == 1) {
		lap = Laplacian19(x, y, z, i32(CabAs), cur, posA);
	} else {
		lap = Laplacian1D(x, y, z, i32(CabAs), cur, posA);
	}
